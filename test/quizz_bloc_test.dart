...
void main() {
  ...
  group('QUIZZ BLOC', () {
    ...
    test('proposition selection & validation', () {
      ...
      bloc.newSelection(q.options.first);
      bloc.quizzState.listen((state) {
        expect(state.selection.length, equals(1));
        expect(state.selection.first, equals(q.options.first));
    ...
    test('multi proposition selection & validation success', () {
      bloc.jump(1);
      final q = mockQuestions.last;
      bloc.newSelection(q.options.toList()[1]);
      bloc.newSelection(q.options.toList()[2]);
